...
Normalizing fixes two problems at once. First, on macOS the concrete platform records the Darwin major version, `arm64-darwin-27` above. That number differs between macOS releases, so lockfiles generated on different Macs disagree about the platform for no useful reason. Normalizing rewrites the entry to the versionless `arm64-darwin`, which covers every macOS release, and later installs keep that form. Second, for gems that ship precompiled platform-specific versions, normalizing adds every platform the locked versions are precompiled for in one step, instead of one `--add-platform` per deploy target. A lockfile freshly generated by Bundler 4 already locks all of those platforms, and normalizing brings an older lockfile to the same state.
