...
The search index is generated by [Pagefind](https://pagefind.app/) as a
post-processing step over the built site, so the search box stays empty
under `rake server`. To try search locally, run:

    $ bundle exec jekyll build
    $ npx --yes pagefind@1.5.2 --site _site --serve
