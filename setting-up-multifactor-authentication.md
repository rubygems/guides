...
You may enable MFA using [WebAuthn](#setting-up-webauthn-recommended), which is
the strongest option, or by using [one-time passwords (OTP)](#setting-up-otp).
...
login to rubygems.org (see [Using recovery codes](#using-recovery-codes)).
