...
with this name when it loads `hola/hola.so`. It is best to adhere to these
naming conventions.
...
First, we can see the `Makefile` generated by `extconf.rb` in `ext/hola`.
Second, we can see two copies of `hola.so`: one in `ext/hola` and the other in
`lib/hola`. When rubygems installs the gem, it copies `hola/hola.so` from `ext`
to `lib`. The `lib` folder is on the `$LOAD_PATH`, so

    require 'hola/hola'

loads `lib/hola/hola.so`. The reason that we don't specify the file extension in
the `require` is that it is platform-dependent.

We can now use the gem:
